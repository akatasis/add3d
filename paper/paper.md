---
title: "Modelling with Code Alone: add.py 2.0, a Single-File Python Library with Dependency-Free Mesh Booleans for a Second-Year Algorithms Course"
...
This paper describes version 2.0: about 3 570 lines, 130 public names, still a single file importing only `math` and `random`. The substantive additions are:
...
Growing to 130 public names is a real cost — a larger surface to learn. We mitigate it by grouping the names into the three layers above, so a student needs only the Draw layer to produce something, and by shipping seventeen numbered example scripts, each demonstrating one group as a runnable program that produces a model file and a rendered image.
