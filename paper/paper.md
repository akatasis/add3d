---
title: "Modelling with Code Alone: add.py 2.x, a Single-File Python Library with Dependency-Free Mesh Booleans for a Second-Year Algorithms Course"
...
This paper describes the 2.x line — version 2.0, which introduced the Booleans, and the 2.1 refinements made after a review of the models students actually build: about 4 700 lines, 178 public names, still a single file importing only `math` and `random`. The substantive additions are:
...
8. **Ready-made parts, placement functions and colour functions** (version 2.1), derived from a survey of the recurring sub-problems in student models (Section 4.7).
...
Faces are polygons, not triangles. A quad grid, which is what every parametric surface produces, reads better in a file, halves the face count, and is supported natively by OFF. Triangulation happens only where it is needed — inside the Boolean evaluator, and in the STL writer.
...
| `sphere(c, 1, 24)` (quad sphere) | V = 4/3·π·r³ | 1 % |
...
| five Platonic solids | face count, V > 0, closed | — |

Beyond volumes, the suite covers colour parsing (named, hexadecimal, 0–255, 0–1, `None`), the scene/layer protocol, transforms (including that mirroring and negative scaling keep the volume positive), arrays and patterns, all four repair passes, file round-trips for OFF, OBJ, PLY and STL, loading legacy and colourless OFF files, and the 1.2 compatibility names.
...
We thank the students of *Algorithm Design and Analysis* at the Faculty of Mathematics and Informatics, Vilnius University, whose models — and whose bug reports — shaped every version of this library, and whose 1.2-era scripts served as the backwards-compatibility test suite for version 2.0.
