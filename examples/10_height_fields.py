...
show("sin(x) + cos(z)", lambda x, z: math.sin(x) + math.cos(z), "red")
show("sin(x) * cos(z)", lambda x, z: math.sin(x) * math.cos(z), "orange")
show("ripple", lambda x, z: math.sin(3 * math.sqrt(x * x + z * z))
     / (1 + x * x + z * z) * 4, "gold")
show("saddle x^2 - z^2", lambda x, z: (x * x - z * z) / 4.0, "lime")
show("monkey saddle", lambda x, z: (x ** 3 - 3 * x * z * z) / 8.0, "teal")
show("gaussian hill", lambda x, z: 3 * math.exp(-(x * x + z * z) / 3.0), "sky")
show("peaks", lambda x, z:
     3 * (1 - x) ** 2 * math.exp(-x * x - (z + 1) ** 2) / 3
     - 10 * (x / 5 - x ** 3 - z ** 5) * math.exp(-x * x - z * z) / 3
     - math.exp(-(x + 1) ** 2 - z * z) / 9, "navy")
show("sombrero", lambda x, z: 3 * math.sin(math.sqrt(x * x + z * z) + 1e-9)
     / (math.sqrt(x * x + z * z) + 1e-9), "purple")
show("checkerboard", lambda x, z: 0.8 * (1 if (int(math.floor(x)) +
                                               int(math.floor(z))) % 2 else -1),
     "magenta", detail=64, thickness=0.1)
show("|x| + |z| cone", lambda x, z: -(abs(x) + abs(z)) / 2.0, "brown")
show("interference", lambda x, z:
     (math.sin(4 * math.sqrt((x - 1.2) ** 2 + z * z))
      + math.sin(4 * math.sqrt((x + 1.2) ** 2 + z * z))), "silver")
show("plateau", lambda x, z: 2.0 / (1 + math.exp(-4 * (2.0 - math.sqrt(
    x * x + z * z)))), "pink")
