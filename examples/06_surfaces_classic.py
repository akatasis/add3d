...
show("hyperboloid", lambda c: add.parametric(
    lambda u, v: [math.cosh(v) * math.cos(u), math.sinh(v),
...
show("helicoid", lambda c: add.parametric(
    lambda u, v: [v * math.cos(u), 0.45 * u, v * math.sin(u)],
    0, 3 * TAU, 120, -1.2, 1.2, 10, c, thickness=0.06), "purple")

show("catenoid", lambda c: add.parametric(
    lambda u, v: [math.cosh(v) * math.cos(u), v, math.cosh(v) * math.sin(u)],
...
show("egg box", lambda c: add.parametric(
    lambda u, v: [u, math.sin(u) * math.cos(v), v],
...
def knot(u, v):
    ...
    h = 1e-4
    t = [((2 + math.cos(q * (u + h))) * math.cos(p * (u + h)) - x) / h,
         (-math.sin(q * (u + h)) - y) / h,
         ((2 + math.cos(q * (u + h))) * math.sin(p * (u + h)) - z) / h]
    n = math.sqrt(sum(a * a for a in t))
    t = [a / n for a in t]
    e1 = [-t[2], 0, t[0]]
    n1 = math.sqrt(sum(a * a for a in e1)) or 1.0
    e1 = [a / n1 for a in e1]
    e2 = [t[1] * e1[2] - t[2] * e1[1], t[2] * e1[0] - t[0] * e1[2],
          t[0] * e1[1] - t[1] * e1[0]]
    return [x + r * (e1[0] * math.cos(v) + e2[0] * math.sin(v)),
            y + r * (e1[1] * math.cos(v) + e2[1] * math.sin(v)),
            z + r * (e1[2] * math.cos(v) + e2[2] * math.sin(v))]
...
show("pseudosphere", lambda c: add.parametric(
    lambda u, v: [math.cos(v) / math.cosh(u), u - math.tanh(u),
                  math.sin(v) / math.cosh(u)],
