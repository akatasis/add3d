...
show("vase", lambda c: lathe(lambda t: [1.1 + 0.55 * math.sin(1.2 * t)
                                        - 0.18 * t, t], c), "teal")
...
def glass(t):
    ...
    if t < 1.6:
        return [0.12, t]                     # stem
    s = (t - 1.6) / 2.4
    return [0.12 + 1.15 * math.sqrt(max(0.0, s)) * (1 - 0.25 * s), t]
...
# --- chess pieces ----------------------------------------------------------
def pawn(t):
    r = (0.85 * math.exp(-3.0 * t) + 0.22
         + 0.42 * math.exp(-28 * (t - 1.35) ** 2)
         + 0.36 * math.exp(-40 * (t - 2.15) ** 2))
    return [r, t]
...
def rook(t):
    if t < 0.35:
        return [0.85 - 0.5 * t, t]
    if t < 1.9:
        return [0.45 + 0.1 * math.exp(-4 * (t - 0.35)), t]
    if t < 2.15:
...
def queen(t):
    r = (0.95 * math.exp(-2.6 * t) + 0.2
         + 0.45 * math.exp(-30 * (t - 1.5) ** 2)
         + 0.30 * math.exp(-60 * (t - 2.5) ** 2)
         + 0.55 * math.exp(-45 * (t - 3.1) ** 2))
    return [r, t]
...
show("doughnut", lambda c: add.revolve(
    lambda t: [2 + math.cos(t), math.sin(t)], [0, 0, 0], [0, 1, 0],
    0, 2 * math.pi, 40, 72, c), "orange")

# --- a fluted column: the profile wobbles ----------------------------------
show("beaded column", lambda c: lathe(
    lambda t: [0.55 + 0.2 * abs(math.sin(2.4 * t)), t], c, 5.0, 220), "purple")

# --- a spinning top --------------------------------------------------------
show("spinning top", lambda c: lathe(
    lambda t: [1.0 * math.sin(math.pi * min(1.0, t / 1.6)) ** 0.6
               if t < 1.6 else max(0.04, 0.35 - 0.3 * (t - 1.6)), t],
    c, 2.6, 140), "red")

# --- cut open to show the wall ---------------------------------------------
show("cup, cut open", lambda c: lathe(
    lambda t: [1.0 + 0.1 * t, t], c, 2.4, 40, angle=1.55 * math.pi), "magenta")

# --- a screw-like profile --------------------------------------------------
show("ribbed pot", lambda c: lathe(
    lambda t: [1.0 + 0.35 * math.sin(3.0 * t) * math.exp(-0.25 * t), t],
    c, 3.6, 180), "navy")
